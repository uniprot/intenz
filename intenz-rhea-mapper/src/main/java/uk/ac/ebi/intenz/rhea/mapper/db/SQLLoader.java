...
public final class SQLLoader {
     ...
     * <code>&lt;aClass&gt;</code> being the fully qualified name of the
     * parameter aClass</li>
     * <li>It is actually a Java properties file, the keys being SQL double-dash
     * comments (these are convenient while editing).</li>
     ...
     * <li>Being a properties file, don't forget the \ character in case the
     * key/value pair spans more than one line!</li>
     * </ul>
     * Example:
     * <pre>
     * --my.insert.1:\
...
//                .getResourceAsStream("uk.ac.ebi.intenz.rhea.mapper.db.IntEnzRheaCompoundDbReader.sql"));   
     ...
     * there, it uses the connection to prepare it and stores it for later use.
     *
     * @param key the key used in the SQL file as a comment (see
     * {@link #SQLLoader(Class, Connection)}).
     * @return a prepared statement (can be a {@link CallableStatement} if the
     * SQL is recognised as such).
     * @throws SQLException
     */
    ...
    /**
     * Retrieves the prepared statement from the internal map. If not already
     * there, this method substitutes any passed parameters, uses the connection
     * to prepare it and stores it for later use.<br/>
     ...
     * MessageFormat documentation). These are useful to define for example the
     * database schema we are working with:      <pre><code>
     *      --my.select.3:\
     ...
     *      </code></pre>
     *
     * Other keys in the SQL file can be used as parameters here, for example to
     * introduce some constraint to a query:<br/>
     * <pre><code>
     *      --my.select.4:\
     ...
     *          "--my.constraint.1");
     *      </code></pre>
     * <b>Warning:</b> single quotes in this kind of parameterised queries
     * should be escaped (using another quote) so that MessageFormat won't break
     * the final SQL syntax. Example:
     * <pre><code>
     *      --my.select.5:\
     ...
     * can be parameterised (see
     * {@link #getPreparedStatement(String, Object[])}).
     *
     * @param key the key used in the SQL file as a comment (see
     * {@link #SQLLoader(Class, Connection)}).
     * @param returnedKeys the column names of the auto-generated keys. They can
     * be retrieved later from the ResultSet Statement.getGeneratedKeys() [NOTE:
     * it seems to work only using column indexes, not names, i.e. even if
     * <code>returnedKeys = { "foo" }</code>, use
     * <code>stm.getGeneratedKeys().getString(1)</code> instead of
    ...
    /**
     * Changes the connection used to prepare statements, after having closed
     * any open ones.
     *
