...
public class ExporterApp {
     ...
     * {@link uk.ac.ebi.intenz.tools.export.XmlExporter XmlExporter} class.</li>
     * <li>Site map XML file (<code>sitemap.xml</code>) to be used in
     * <a href="http://www.google.com/webmasters/sitemaps">Google sitemaps</a>
     * to make every IntEnz entry available to Google indexing. Other search
     * engines accept this standard too.</li>
     ...
     * @param args
     * <ul>
     * <li>-intenzDb &lt;config&gt;: database configuration file for
     * IntEnz.</li>
     ...
     * <li>[-ec &lt;EC number&gt;]: export only the passed EC number. if not
     * set, all of the public entries are exported.</li>
     * </ul>
...
//                .getConnection();
      Connection iconnection  = getConnectionInstance(dbConfig);
        stats = new IntEnzDbStatistics(iconnection);
    }
    ...
    protected void exportKegg(Collection<EnzymeEntry> enzymes, String keggFile)
        ...
        try {
            ...
            if (!keggEnzymeFile.exists()) {
                keggEnzymeFile.createNewFile();
            }
