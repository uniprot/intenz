...
//  private static final Logger LOGGER = Logger.getLogger(EnzymeEntryHelper.class);
  ...
  public static String toXML(EnzymeSubSubclass enzymeSubSubclass, SpecialCharacters encoding) {
    StringBuffer xmlStringBuffer = new StringBuffer();

    xmlStringBuffer.append("<enzyme>");

    xmlStringBuffer.append("<ec>");
    xmlStringBuffer.append("EC " + enzymeSubSubclass.getEc().toString());
    xmlStringBuffer.append("</ec>");
    ...
    if (!enzymeSubSubclass.getName().equals("")) {
      xmlStringBuffer.append(encoding.xml2Display(removeFormatting(enzymeSubSubclass.getName())));
    }
    xmlStringBuffer.append("</common_name>");

    if (!enzymeSubSubclass.getName().equals("")) {
      xmlStringBuffer.append("<common_name>");
      xmlStringBuffer.append(encoding.xml2Display(removeFormatting(enzymeSubSubclass.getName()), EncodingType.SWISSPROT_CODE));
      xmlStringBuffer.append("</common_name>");
    }

    xmlStringBuffer.append("<description>");
    if (!enzymeSubSubclass.getDescription().equals("")) {
      xmlStringBuffer.append(encoding.xml2Display(removeFormatting(enzymeSubSubclass.getDescription())));
    }
    xmlStringBuffer.append("</description>");

    if (!enzymeSubSubclass.getDescription().equals("")) {
      xmlStringBuffer.append("<description>");
      xmlStringBuffer.append(encoding.xml2Display(removeFormatting(enzymeSubSubclass.getDescription()), EncodingType.SWISSPROT_CODE));
      xmlStringBuffer.append("</description>");
    }
