 ...
 *
 * @author Michael Darsow, Rafael Alcántara,Joseph Onwubiko
...
public class EnzymeReactionMapper {
     ...
     * <code>Reaction</code>instances or <code>null</code> if nothing has been
     * found.
    ...
    public EnzymaticReactions find(Long enzymeId, Connection con)
        ...
        if (result == null) { // no Rhea reactions
            ...
            if (abstractReactions != null) {
                result.add(abstractReactions);
            }
        }
        return result;
    }
     ...
     * @return an <code>EnzymaticReactions</code> containing reactions or
     * <code>null</code> if no reaction could be found.
    ...
    public void insert(EnzymaticReactions reactions, Long enzymeId, Connection con)
            throws SQLException {
        if (reactions == null) {
            throw new NullPointerException("Parameter 'reactions' must not be null.");
        }
        if (enzymeId == null) {
            throw new NullPointerException("Parameter 'enzymeId' must not be null.");
        }
    ...
    /**
     * Replace the reactions in the database related to one enzyme with those
     * passed as parameter.
     *
     * @param enzymeId The enzyme ID.
     * @param reactions The vector of reactions.
    ...
    /**
     * Deletes all reactions mappings and abstract reactions related to one
     * enzyme instance.
     *
    ...
    /**
     * Creates an empty Reaction object, with just an equation, source and
     * status. The reaction ID will be <code>null</code> for old plain text
     * reactions, not <code>null</code> in case of error while reading data from
     * Rhea.
     *
     * @param reactionId
     * @param equation
    ...
    private Reaction loadReaction(ResultSet rs) throws SQLException, MapperException {
        ...
        if (status.equals("SU") || status.equals("PR")) {
            status = "NO";
        }
        Reaction reaction;
