...
public class PopulateIntEnzEnzymeDTOAction extends Action {
    ...
    // Check if an entry had to be reloaded and forward to the page where this entry has been displayed before
        ...
        if (request.getParameter("reload") != null) {
            reload = request.getParameter("reload").equals("") ? false : true;
        }
        if (reload) {
        ...
        if (enzymeEntry.getHistory().isDeletedRootNode()) {
            return mapping.findForward(DELETED_INTENZ_ENTRY_JSP_FWD);
        }
        if (enzymeEntry.getHistory().isTransferredRootNode()) {
            return mapping.findForward(TRANSFERRED_INTENZ_ENTRY_JSP_FWD);
        }
    ...
    private String getXcharsView(HttpServletRequest request) {
        ...
        if (xcharsViewString == null) {
            xcharsViewString = "false";
        }
...
//      enzymeDTO.setLatestHistoryEventClass(EventConstant.CREATION.toString());
        ...
        Long beforeId = null;
        if (beforeNode != null) {
            beforeId = beforeNode.getEnzymeEntry().getId();
        }
        if (beforeId != null) {
            enzymeDTO.setLatestHistoryBeforeId(beforeId.toString());
        }
        final HistoryNode afterNode = enzymeEntry.getHistory().getLatestHistoryEventOfRoot().getAfterNode();
        Long afterId = null;
        if (afterNode != null) {
            afterId = afterNode.getEnzymeEntry().getId();
        }
        if (afterId != null) {
            enzymeDTO.setLatestHistoryAfterId(afterId.toString());
        }
        ...
        for (Object o : cofactors) {
            ...
            if (o instanceof Cofactor) {
                ...
                while (c == null) {
                    ...
                    if (obj instanceof Cofactor) {
                        c = (Cofactor) obj;
                    } else {
                        loopOs = (OperatorSet) obj;
                    }
                }
                dto.setSource(c.getSource().toString());
                dto.setView(c.getView().toString());
    ...
    private List getLinksList(SortedSet links, boolean getUniProtLinksOnly, String ec, SpecialCharacters encoding,
        ...
        for (Iterator it = links.iterator(); it.hasNext();) {
            ...
            enzymeLinkDTO.setName(enzymeLink.getName());
